...
export const CreateItemPage: React.FC<CreateItemPageProps> = ({ onItemCreated }) => {
  ...
  const [category, setCategory] = useState(categories[0]); // ★ カテゴリー用のstateを追加
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    formData.append('category', category); // ★ カテゴリーをFormDataに追加
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.message || '商品出品に失敗しました。');
      }
      await onItemCreated();
    } catch (error) {
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className={styles.formGroup}>
            ...
            <select
              ...
              className={styles.selectInput} // ★ 後でCSSを追加
            >
              {categories.map(cat => (
          ...
          <div className={styles.formGroup}>
            <TextArea
              placeholder="商品の説明（色、素材、サイズ、状態など）"
              rows={6}
              value={description}
